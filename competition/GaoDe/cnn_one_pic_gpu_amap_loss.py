...
class amap_loss(nn.Module):
    ...
    def backward(self,output, labels):
        ...
        return torch.tensor(loss)
    # def backward(self):
    #     if pd.isnull(self.f1_0):
    #         self.f1_0 = 0
    #     if pd.isnull(self.f1_1):
    #         self.f1_1 = 0
    #     if pd.isnull(self.f1_2):
    #         self.f1_2 = 0
    #     # print('f1', self.f1_0, self.f1_1, self.f1_2)
    #     loss = 1 - 0.2 * self.f1_0 - 0.2 * self.f1_1 - 0.6 * self.f1_2
    #     # print(loss)
    #     return loss
...
def train(train_dataset, val_dataset, batch_size, epochs, learning_rate, wt_decay):
    ...
    for epoch in range(epochs):
        ...
        for images, labels in train_loader:
            ...
            loss_rate = amap_loss().cpu()
            # 误差的反向传播
            loss_rate.backward(output, labels)
        ...
        if epoch % 5 == 0:
            ...
            print('Epoch: ', epoch, '| loss_rate: %.8f' % loss_rate.backward(output, labels), '| acc_train: %.4f' % acc_train,
                  '| val accuracy: %.4f' % acc_val, '|', time_format)
        else:
            print('Epoch: ', epoch, '| loss_rate: %.8f' % loss_rate.backward(output, labels))

        # if loss_rate.backward() < 0.01:
        #     return model
...
def main():
    ...
    # 超参数可自行指定
    model = train(train_dataset, val_dataset, batch_size=56, epochs=45, learning_rate=0.1, wt_decay=0)
    # 保存模型
